#!/usr/bin/env Rscript
# WRF-STILT v3.2 — release at 06 UTC (= 13 WIB, true afternoon mixed PBL)
#
# Replaces v3.1 12 UTC (= 19 WIB, post-sunset stable layer) configuration.
# Aligns with OCO-2 overpass time (~06 UTC) for joint assimilation.
#
# Output directory: out_06utc/ (kept separate from the 12 UTC run)
# Receptors: 11 sites × 26 days (Oct 6-31, 06 UTC) = 286 simulations

# ── Paths (must be set before source('r/dependencies.r')) ─────────────────────
stilt_wd  <- '/home/igrk/WRF-GRK/stilt'
output_wd <- file.path(stilt_wd, 'out_06utc')
lib.loc   <- .libPaths()[1]
...
# ── Receptors ─────────────────────────────────────────────────────────────────
# Read from receptors_full_oct_06utc.csv: run_time (06 UTC), lati, long, zagl
receptors <- read.csv(file.path(stilt_wd, 'receptors_full_oct_06utc.csv'),
...
# skipped: do not wipe existing footprints
if (run_trajec) {
}
